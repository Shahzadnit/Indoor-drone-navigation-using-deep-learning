     ...
     if(88<=theta & theta<=93)
       ...
       y2=(Center_y-y1)*l+Center_y;
        ...
        TrainImgNo = TrainImgNo+1;
                 xtest(:,:,:,TrainImgNo) = permute(I3,[2 1 3]);
                ytest(1,1,TrainImgNo) = lebel(i+1,1);
                ytest(1,2,TrainImgNo) = lebel(i+1,2);
                ytest(2,1,TrainImgNo) = lebel(i+1,3);
                ytest(2,2,TrainImgNo) = lebel(i+1,4);
                ytest(3,1,TrainImgNo) = lebel(i+1,6)*(pi/180);
                ytest(3,2,TrainImgNo) = lebel(i+1,1)/320;
               
               
               filename = sprintf('./TestResults/%d.png', TrainImgNo);
                imwrite(I3, filename);
                
               flip_I3 = fliplr(I3);
          
                TrainImgNo = TrainImgNo+1;
                xtest(:,:,:,TrainImgNo) = permute(flip_I3,[2 1 3]);
                ytest(1,1,TrainImgNo) = 320-lebel(i+1,1);
                ytest(1,2,TrainImgNo) = lebel(i+1,2);
                ytest(2,1,TrainImgNo) = 320-lebel(i+1,3);
                ytest(2,2,TrainImgNo) = lebel(i+1,4);
                ytest(3,1,TrainImgNo) = pi-lebel(i+1,6)*(pi/180);
                ytest(3,2,TrainImgNo) = 1-lebel(i+1,1)/320;
